...
function kaliTerusRekursif(angka) {
  if (angka <= 9) {
    ...
    if (String(hasil).split("").length >= 2) {
      for (let i = 0; i < String(hasil).split("").length; i++) {
        hasil = (hasil % 10) * kaliTerusRekursif(Math.floor(hasil / 10));
        if (String(hasil).split("").length == 1) {
          return hasil;
        }
      }
    } else {
